...
static sgx_status_t SGX_CDECL sgx_ecall_process_request(void* pms)
{
	...
	if (_tmp_resp != NULL && _len_resp != 0) {
		_in_resp = (struct response_msg*)malloc(_len_resp);
		if (_in_resp == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_resp, _tmp_resp, _len_resp);
	}
...
static sgx_status_t SGX_CDECL sgx_ecall_shutdown_sg(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ecall_shutdown_sg();
...
static sgx_status_t SGX_CDECL sgx_ecall_init_sg(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	void* _tmp_config = ms->ms_config;
	size_t _tmp_config_len = ms->ms_config_len;
	size_t _len_config = _tmp_config_len;
	void* _in_config = NULL;
	...
	if (_tmp_config != NULL && _len_config != 0) {
		_in_config = (void*)malloc(_len_config);
		if (_in_config == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	ms->ms_retval = ecall_init_sg(_in_config, _tmp_config_len);
err:
...
SGX_EXTERNC const struct {
	...
	12,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_shutdown_sg, 0},
