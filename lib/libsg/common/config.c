...
int verify_config(configuration *config) { return 1; }
...
configuration *unpack_config(void *buf, size_t len) {
  ...
  // memcpy(config->sealed_sg_ctx_file, c->sealed_sg_ctx_file,
  // strlen(c->sealed_sg_ctx_file)+1);
  config->sealed_sg_ctx_file =
      strndup(c->sealed_sg_ctx_file, strlen(c->sealed_sg_ctx_file) + 1);
...
void *pack_config(configuration *config, size_t *out_len) {
  ...
  c.sealed_sg_ctx_file =
      config->sealed_sg_ctx_file; // strdup(config->sealed_sg_ctx_file);
...
char *serialize_config(configuration *config, size_t *len) {
  ...
  *len = sofar;
  ...
  for (i = 0; i < config->found_ips; ++i) {
    ...
    memcpy(buf + sofar, config->ips[i],
           strlen(config->ips[i]) + 1); // Copy null terminator
    sofar += strlen(config->ips[i]) + 1;
  }
  // printf("sofar = %d\n", sofar);
