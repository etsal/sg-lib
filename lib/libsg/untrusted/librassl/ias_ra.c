...
static void http_get(CURL *curl, const char *url,
    struct buffer_and_size *header, struct buffer_and_size *body,
    struct curl_slist *request_headers, char *request_body);
static size_t accumulate_function(
    void *ptr, size_t size, size_t nmemb, void *userdata);
static void extract_certificates_from_response_header(CURL *curl,
    const char *header, size_t header_len,
    attestation_verification_report_t *attn_report);
// static void base64_encode(uint8_t *in, uint32_t in_len,
//  uint8_t* out, uint32_t* out_len /* in/out */);
static void parse_response_header(const char *header, size_t header_len,
    unsigned char *signature, const size_t signature_max_size,
...
*/
void
obtain_attestation_verification_report(const sgx_quote_t *quote,
    const uint32_t quote_size, const ra_tls_options_t *opts,
    attestation_verification_report_t *attn_report)
{
	int ret;
	...
	snprintf(json, strlen(json_template) + quote_base64_len + 5,
	json_template, quote_base64);
	*/
	base64_encode_wbuf((unsigned char *)quote, quote_size, quote_base64,
	    (size_t *)&quote_base64_len);

	memset(json, 0, quote_size * 2);
	strcat(json, "{\n\"isvEnclaveQuote\":\"");
	strcat(json, quote_base64);
	strcat(json, "\"\n}");

#if DEBUG_IAS_RA
	edividerWithText("json");
	eprintf("%s\n", json);
	edivider();
#endif
...
#ifdef DEBUG_IAS_RA
	edividerWithText("IAS Report");
	eprintf("+++ header:\n%s\n", header.data);
	eprintf("+++ body:\n%s\n", body.data);
	edivider();
#endif

	// The header contains the signature
	...
	// The body contains the report (which is the quote structure without
	...
	extract_certificates_from_response_header(
	    curl, header.data, header.len, attn_report);
...
size_t
accumulate_function(void *ptr, size_t size, size_t nmemb, void *userdata)
{
...
void
http_get(CURL *curl, const char *url, struct buffer_and_size *header,
...
void
extract_certificates_from_response_header(CURL *curl, const char *header,
    size_t header_len, attestation_verification_report_t *attn_report)
{
	...
	char *field_begin = memmem(header, header_len, response_header_name,
	...
	char *field_end = memmem(field_begin,
	    header_len - (field_begin - header), http_line_break,
	...
	char *unescaped = curl_easy_unescape(
	    curl, field_begin, field_len, &unescaped_len);

	char *cert_begin = memmem(unescaped, unescaped_len, pem_marker_begin,
	    strlen(pem_marker_begin));
	assert(cert_begin != NULL);
	char *cert_end = memmem(
	    unescaped, unescaped_len, pem_marker_end, strlen(pem_marker_end));
	...
	cert_begin = memmem(cert_end, unescaped_len - (cert_end - unescaped),
	    pem_marker_begin, strlen(pem_marker_begin));
	assert(cert_begin != NULL);
	cert_end = memmem(cert_begin, unescaped_len - (cert_begin - unescaped),
...
void
parse_response_header(const char *header, size_t header_len,
    unsigned char *signature, const size_t signature_max_size,
    uint32_t *signature_size)
{

	const char sig_tag[] = "X-IASReport-Signature: ";
	char *sig_begin = memmem(
	    (const char *)header, header_len, sig_tag, strlen(sig_tag));

	assert(sig_begin != NULL);
	sig_begin += strlen(sig_tag);
	char *sig_end = memmem(sig_begin, header_len - (sig_begin - header),
	    "\r\n", strlen("\r\n"));
...
void
tmp_base64_encode(
    uint8_t *in, uint32_t in_len, uint8_t *out, uint32_t *out_len /* in/out */)
{
	// + 1 to account for the terminating \0.
	assert(*out_len >= (in_len + 3 - 1) / 3 * 4 + 1);
	bzero(out, *out_len);

	int ret = 0; // Base64_Encode_NoNl(in, in_len, out, out_len);
	assert(ret == 0);
