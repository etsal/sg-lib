...
typedef struct ms_initialize_sg_t {
	char* ms_filename;
...
{
	CHECK_REF_POINTER(pms, sizeof(ms_initialize_sg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_initialize_sg_t* ms = SGX_CAST(ms_initialize_sg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	initialize_sg((const char*)_in_filename);
err:
...
sgx_status_t SGX_CDECL ocall_load_config(int* retval, const char* filename, config_ctx_t* ctx)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_filename = filename ? strlen(filename) + 1 : 0;
	size_t _len_ctx = sizeof(config_ctx_t);
	...
	if (ctx != NULL && sgx_is_within_enclave(ctx, _len_ctx)) {
		ms->ms_ctx = (config_ctx_t*)__tmp;
		memcpy(__tmp, ctx, _len_ctx);
