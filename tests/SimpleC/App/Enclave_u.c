...
static sgx_status_t SGX_CDECL Enclave_ocall_load_config(void* pms)
{
	...
	ms->ms_retval = ocall_load_config((const char*)ms->ms_filename, ms->ms_ctx);
...
sgx_status_t initialize_sg(sgx_enclave_id_t eid, const char* filename)
{
	sgx_status_t status;
	ms_initialize_sg_t ms;
