 ...
 */
int 
br_ec_key_gen_curve25519(br_ec_key *pkey, br_ec_key *skey)
{
	*pkey = BR_EC_KEY_INIT_PKEY;
	*skey = BR_EC_KEY_INIT_SKEY;
...
{
	if (key->type == PKEY && key->pkey.q != nullptr)
		ZERO_AND_FREE(key->pkey.q, key->pkey.qlen);

	else if (key->type == SKEY && key->skey.x != nullptr)
		ZERO_AND_FREE(key->skey.x, key->skey.xlen);
}

int 
br_ec_key_gen_static(br_ec_key *pkey, br_ec_key *skey)
{
	*pkey = BR_EC_KEY_INIT_PKEY;
	*skey = BR_EC_KEY_INIT_SKEY;
	...
	pkey->pkey.curve = curve;
	pkey->pkey.q = public_key;
	pkey->pkey.qlen = sizeof(public_key);

	skey->skey.curve = curve;
	skey->skey.x = private_key;
	skey->skey.xlen = sizeof(private_key);

	return 0;
}

void 
br_ec_key_print(br_ec_key *key)
{
	if (key->type == PKEY) {
		printf("Private Key: ");
		for (int i=0; i<key->pkey.qlen; ++i)
			printf("%02x ", key->pkey.q[i]);
	}
	else {
		printf("Secret Key: ");
		for (int i=0; i<key->skey.xlen; ++i)
			printf("%02x ", key->skey.x[i]);
	}
...
gen_key_pair(br_ec_public_key *pk, br_ec_private_key *sk, int curve, void *impl)
{
	...
	if (br_ec_keygen(&rng_ctx.vtable, (const br_ec_impl *)impl, 
	...
	if (br_ec_compute_pub((const br_ec_impl *)impl, pk, kbuf_pub, sk) == 0) {
#if DEBUG
