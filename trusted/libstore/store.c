...
static void create_entry(entry_t **entry, const char *key, const void *value, size_t value_len);
 ...
 */
void
init_store(table_t *table, uint64_t uid)
{
	table->uid = uid;
	table->entries = NULL;
	if (uid == 0) {
...
int
is_empty_store(table_t *table)
{
	if (table->uid == 0 || table->entries == NULL ||
	    table->versions == NULL)
		return 1;
	return 0;
}
...
static void
create_entry(
    entry_t **entry, const char *key, const void *value, size_t value_len)
{
...
int
put_store(
    table_t *table, const char *key, const void *value, size_t value_len)
{
 	entry_t *entry = NULL;
	uint64_t ts = 0;
 ...
 */
int
get_store(table_t *table, const char *key, void *value, size_t *value_len)
{
    entry_t *entry = NULL;
    uint64_t ts = 0;
 ...
 */
static void
put_entry_store(table_t *table, entry_t *entry)
{
	...
	HASH_ADD_KEYPTR(hh, table->entries, new_entry->key, strlen(new_entry->key), new_entry);
}
...
void
merge_store(table_t *local_set, table_t *remote_set)
{
...
void
print_fmt_store(table_t *table, void (*format)(const void *data))
{
	entry_t *entry;
...
void 
protobuf_pack_store(table_t *table, Table *t)
{
    t->uid = table->uid;
...
void 
protobuf_free_packed_store(Table *t)
{
...
void
protobuf_unpack_store(table_t *table, Table *t)
{
    free(table);
    table->uid = t->uid; 
	for (int i = 0; i < t->n_entries; ++i) {
		...
		protobuf_unpack_vvec(
		    &entry->versions, t->entries[i]->versions);
...
void
serialize_store(table_t *table, uint8_t **buf, size_t *len)
{
...
void
deserialize_store(table_t *table, uint8_t *buf, size_t len)
{
	Table *ptable = NULL;
	...
	for (int i = 0; i < ptable->n_entries; ++i) {
		...
		entry->versions = NULL;
		protobuf_unpack_vvec(
		    &entry->versions, ptable->entries[i]->versions);
