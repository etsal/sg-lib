...
static void add_site_to_device(uint64_t key_handle_dec, br_ec_private_key *skey);
static int get_site(site_info_t **site, uint64_t key_handle);
static void hex_dec(const uint8_t *key_handle, uint64_t *dec);
static void gen_key_handle(br_ec_private_key *sk, unsigned char *buf, size_t len);

static void
prepare_new_site(site_info_t *site, br_ec_key *skey)
{
...
void 
print_site_info(const void *data)
{
...
static int
init_new_device(const char *filename)
{
    ...
    //eprintf("+ Generating self-signed X509 certificate of device attestation key\n");
	...
	if (device_ctx.cert_len > CERT_MAX_LEN) {
		eprintf("Error, generated certificate is too big - expected %d actual %d.\n", CERT_MAX_LEN, device_ctx.cert_len );
		exit(1);
	}
    ...
    ret = add_sg(&device_ctx.sg, 0, (void *)&device_ctx, sizeof(u2fdev_ctx) - sizeof(sg_ctx_t));
    if (!ret) {
        eprintf("Error, failed to add pair <0,device_ctx> to store.\n");
        exit(1);
    }

    ret = save_sg(&device_ctx.sg, filename);
    if (ret) {
...
{
	guid = uid;
    int ret = load_sg(&device_ctx.sg, U2F_FILENAME);
	if (!ret) {
		eprintf("+ Loaded existing table\n");		// Entry with value 0 stores a device_context, load that into device_ctx
		u2fdev_ctx ctx;
		ret = find_sg(&device_ctx.sg, 0, (void *)&ctx, sizeof(u2fdev_ctx) - sizeof(sg_ctx_t));
		if (!ret) {
...
int
ecall_get_cert(unsigned char *buf, size_t len)
{
 ...
 */
void 
ecall_generate_site_keys(unsigned char *key_handle, size_t key_handle_len, 
...
#endif
    ret = add_sg(&device_ctx.sg, kh, (void *)&new_site, sizeof(site_info_t));
    if (!ret) {
        eprintf("Error, failed to add pair to store\n");
        exit(1);
    }

    ret = save_sg(&device_ctx.sg, U2F_FILENAME);
    if (ret) {
        eprintf("%s : failed to save table\n");
        exit(1);
    }
...
//#ifdef DEBUG
    ...
    eprintf("Public Key  (%lu) : %s...\n", public_key_len, hexstring(public_key, 10));
...
//#endif
    ...
    ret = send_update_sg(&device_ctx.sg, hostname_list[(guid+1)%2]);
#ifdef DEBUG_ENC
    if (ret) {
    ...
    print_sg(&device_ctx.sg, &print_site_info);
...
int 
ecall_generate_registration_signature(const unsigned char *key_handle, size_t key_handle_len,
...
int 
ecall_generate_authentication_signature(const unsigned char *key_handle, size_t key_handle_len,
	const char *data, size_t data_len,  
	unsigned char *signature, size_t signature_len)
{
	br_sha256_context hash_ctx;
	...
	site_info_t site;
    hex_dec(key_handle, &kh);
	int ret = find_sg(&device_ctx.sg, kh, (void *)&site, sizeof(site_info_t));
	if (!ret) {
	...
	ret = br_ecdsa_i31_sign_asn1(&br_ec_all_m31, &br_sha256_vtable, buf,
	    &skey, signature);
...
uint32_t 
ecall_inc_and_get_site_counter(const unsigned char *key_handle, size_t key_handle_len)
{
	...
	// Find value
	hex_dec(key_handle, &kh);
    int ret = find_sg(&device_ctx.sg, kh, buf, buf_len);
	...
	ret = remove_sg(&device_ctx.sg, kh);
	if (!ret) {
		eprintf("%s : failed to remove kv-pair\n");
		return -1;
	}

	ret = add_sg(&device_ctx.sg, kh, site, sizeof(site_info_t));
	if (!ret) {
...
int 
ecall_listen_updates()
{
	eprintf("+ Listening for updates\n");
	return listen_updates_sg(&device_ctx.sg);	
}


int 
ecall_send_update(const char *host)
{
...
static void
hex_dec(const uint8_t *key_handle, uint64_t *dec)
{
	int base = 1;
	*dec = 0;
	for (int i = 3; i >= 0; --i) {
...
static void
gen_key_handle(br_ec_private_key *sk, unsigned char *buf, size_t len)
{
	br_sha256_context hash_ctx;
