...
typedef struct {
	uint8_t version[VERSION_STR_LEN];
} U2F_VERSION_RESP;
...
#define U2F_REQ_MSG_SIZE 57
#define U2F_REQ_MSG_MAX_DATA_SIZE (U2F_REQ_MSG_SIZE - 7)
#define U2F_REQ_MSG_DATA_LEN(m) (((m).lc2 << 8) + (m).lc3)
...
typedef struct {
	...
	uint8_t data[U2F_REQ_MSG_SIZE - 7]; // Data field
} U2F_REQ_MSG;
